...
def sum_power_lower_envelope_approximation(
    distance, delta_freq, freq, h_tx, h_rx, c=constants.c, power_tx=1
):
    _factor = power_tx / 4
    _part1 = delta_freq**2 * 0.5 * (1 / freq**2 + 1 / (freq + delta_freq) ** 2)
    _part2 = (h_rx * h_tx / distance**2) ** 2
    return _factor * _part1 * _part2


def sum_power(
    distance, delta_freq, freq, h_tx, h_rx, G_los=1, G_ref=1, c=constants.c, power_tx=1
):
    power_f1 = rec_power(distance, freq, h_tx, h_rx, G_los=G_los, G_ref=G_ref, c=c)
    power_f2 = rec_power(
        distance, freq + delta_freq, h_tx, h_rx, G_los=G_los, G_ref=G_ref, c=c
    )
...
def sum_power_lower_envelope(
    ...
    _part1 = c**2 / (4 * d_los**2) * (1.0 / omega**2 + 1.0 / omega2**2)
    _part2 = c**2 / (4 * d_ref**2) * (1.0 / omega**2 + 1.0 / omega2**2)
    A = (c / (2 * omega)) ** 2
    B = (c / (2 * omega2)) ** 2
    _part3 = (
        -2
        / (d_los * d_ref)
        * np.sqrt(
            A**2 + B**2 + 2 * A * B * np.cos(delta_omega / c * (d_ref - d_los))
        )
    )
...
def main_power_two_freq(
    freq, delta_freq, h_tx, h_rx, plot=False, export=False, **kwargs
):
    ...
    power_sum = 0.5 * (power_rx + power_rx2)
    ...
    power_sum_lower_approx = sum_power_lower_envelope_approximation(
        distance, delta_freq, freq, h_tx, h_rx
    )
    ...
    if export:
        ...
        export_results(
            results,
            f"power_sum_approx-{freq:E}-df{delta_freq:E}-t{h_tx:.1f}-r{h_rx:.1f}.dat",
        )
    return results
