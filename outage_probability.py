...
def get_intersections(intervals, sensitivity, df, freq, h_tx, h_rx):
    ...
    for _d_lower, _d_upper in intervals:
        try:
            _d_intersect = optimize.root_scalar(func_intersect,
...
def main_outage_prob_power(freq, h_tx, h_rx, df: float,
                           c=constants.c, num_samples=100000, plot=False, export=False, **kwargs):
    LOGGER.info(f"Simulating outage probability with parameters: "
    ...
    outage_prob_analytical = [calculate_outage_prob(df, freq, h_tx, h_rx, sens, rv_distance)
                              for sens in threshold]
    approx_out_prob = rv_distance.sf((1./threshold_lin)**(1/4)*np.sqrt(h_tx*h_rx))
    _approx_min_d = 4*np.pi*(freq+df)*h_rx*h_tx/c
    _approx_min_power_exact = (c/(2*np.pi*(freq+df)))**4/(4*h_rx*h_tx)**2
    LOGGER.info(f"The approximation is valid for: s < {to_decibel(_approx_min_power_exact):.1f}dB")

    _approx_min_s = sum_power_lower_envelope(crit_dist(df, h_tx, h_rx, k=1), df, freq, h_tx, h_rx)
    ...
    LOGGER.info(f"The worst-case approximation is valid for: s < {to_decibel(_approx_min_s):.1f}dB")
    ...
    if export:
        ...
        export_results(results, f"out_prob_power-{freq:E}-df{df:E}-t{h_tx:.1f}-r{h_rx:.1f}.dat")
    return results
